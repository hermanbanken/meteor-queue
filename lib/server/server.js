...
Queue.process = function (entry) {
    ...
    function callback(result) {
        Queue._handle(entry, message, history, result);
    }

    try {
        if (entry.async === true) {
            new Function("callback", entry.command)(callback);
        } else {
            callback(new Function(entry.command)());
        }
    } catch (e) {
        message = e.err;
        callback(false);
...
Queue._handle = function (entry, message, history, result) {
    if (result !== false) {
